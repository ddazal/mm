...
export class WelcomeEmailTemplate {
  static text(user: User): string {
    return `Hola, ${user.name}. Bienvenido a Meeting-o-Matic. El mejor lugar para programar tus reuniones sin enviar decenas de correos. Gracias por confiar en nosotros para gestionar los detalles de tus reuniones. Si no sabes de qué estamos hablando y no has usado nuestra plataforma, por favor haz caso omiso a este mensaje. Saludos cordiales, Meeting-o-Matic.`
  }
  static html(user: User): string {
    return `
    <!DOCTYPE html>
    <html>
      <head>
